...
float plan_compute_profile_nominal_speed(plan_block_t *block)
{
    ...
    else
    {
        if (!(block->condition & PL_COND_FLAG_NO_FEED_OVERRIDE))
        {
			...
			//TODO:I dont think we can over ride a feed rate for a g95??
	...
	/*
	TODO:feed rate per rev minimum
	If the drive mode is going to be via encoder pulse for G95, we should/must ignore
	minimum feed rate. MINIMUM_FEED_RATE is usually 1.0, and commonly threading of <1mm
	per rev will take place. For now just return the feedrate per rev value, but this may need revisited.
	*/
	if (! block->condition & PL_COND_FLAG_FEED_REVOLUTION)
