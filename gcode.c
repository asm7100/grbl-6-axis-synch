...
uint8_t gc_execute_line(char *line)
{
        ...
        switch (letter)
        {
            ...
            case 'G':
                ...
                switch (int_value)
                {
					...
					case 95:
                        word_bit = MODAL_GROUP_G5;
						gc_block.modal.feed_rate = FEED_RATE_MODE_UNITS_PER_ROTATION;
                        break;
                    case 20:
                    case 21:
    ...
    else
    {
        ...
        else
        {
			...
			// = G95
			// - In units per rotation mode: If F word passed, ensure value is in units/rotation, otherwise push last state value.
			if (gc_state.modal.feed_rate == FEED_RATE_MODE_UNITS_PER_ROTATION)
			{
				// Last state is also G95
				if (bit_istrue(value_words, bit(WORD_F)))
				{
					if (gc_block.modal.units == UNITS_MODE_INCHES)
					gc_block.values.f *= MM_PER_INCH;
				}
				else //Can we carry the last feed rate to the new block if it wasnt specified, during a G95?
				gc_block.values.f = gc_state.feed_rate; // Push last state feed rate
			}
    ...
    if (bit_istrue(command_words, bit(MODAL_GROUP_G12)))
    {
        ...
        if (gc_state.modal.coord_select != gc_block.modal.coord_select
                && !(settings_read_coord_data(gc_block.modal.coord_select, block_coord_system)))
	...
	if (gc_state.modal.feed_rate == FEED_RATE_MODE_UNITS_PER_ROTATION)
		pl_data->condition |= PL_COND_FLAG_FEED_REVOLUTION; // Set condition flag for planner use.
    ...
    if ((gc_state.spindle_speed != gc_block.values.s) || bit_istrue(gc_parser_flags, GC_PARSER_LASER_FORCE_SYNC))
    {
        if (gc_state.modal.spindle != SPINDLE_DISABLE && bit_isfalse(gc_parser_flags, GC_PARSER_LASER_ISMOTION))
            spindle_sync(gc_state.modal.spindle, bit_istrue(gc_parser_flags, GC_PARSER_LASER_DISABLE) ? 0.0 : gc_block.values.s);
    ...
    if (gc_state.modal.program_flow)
    {
        ...
        if (gc_state.modal.program_flow == PROGRAM_FLOW_PAUSED && sys.state != STATE_CHECK_MODE)
        {
