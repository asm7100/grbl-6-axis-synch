...
typedef struct
{
	...
	uint8_t drive_mode; // 0= drive from the timer tick, 1= drive from encoder pulse on spindle
} st_block_t;
...
void st_wake_up()
{
	...
	/*
	Drive mode will be 0 by default, which will allow normal execution of the stepper block
	by allowing the timer tick to execute the steps. However in G95 feed mode, the execution
	of the block data will be controlled by the pulse of an encoder on the spindle. This should
	allow spindle synch to operate by only causing motion when a pulse arrives from the encoder.
	**Changed this AGAIN... the spindle encoder is read by the spindle controller board. The
	spingle controller board sends pulses to grbls systme pin. The encoder does NOT directly
	pulse the system pin now. 
	*/
	//Enable ISR timer if in g93 or g94 feed mode.
	if (st_prep_block->drive_mode == 0)
	{
...
void st_drive()
{
	...
	if (st.exec_segment == NULL)
	{
		...
		if (segment_buffer_head != segment_buffer_tail)
		{
			...
			//In G95 mode this value will not actualy get used. Its set, but the timer is disabled
			//so this value has no meaning really. 
	...
	{ // Check if we need to fill the buffer.
		...
		if (pl_block == NULL)
		{
			...
			else
			{
				...
				//Here a plan block is broken up into 'segments'. Each segment is a fragment of the plan blocks
				//motion. 
				//'prep' is a struct that holds data that can be reused to plan each segment from the plan block. 
				//prep.st_block_index holds the index number for the segment buffer. That way the next time this
				//loops around we dont load a new block, we jsut create another segment from the plan block
				//This repeats until the entire block has been processed as a set of segments. Then we load another
				//plan block. 
				...
				//There really isnt much data we need to move to the st_prep_block. 
		...
		/*
		If G95 feed mode was specified in the block, set the drive mode for the stepper execution
		to be controlled by an encoder pulse. If any other feed mode mode is active, the steppers
		are driven by the timer tick for the stepper ISR, because drive_mode = 0.
				
		I do now wonder if we could by-pass all the acceleration calculations for G95 feed mode
		because the acceleration of the spindle is what will control the acceleration of the axes
		in motion. There will have to be considerations taken for spindle acceleration that
		exceeds the possible acceleration of the axes hardware. 
		*/
		
		if (pl_block->condition & PL_COND_FLAG_FEED_REVOLUTION)					
			st_prep_block->drive_mode = bit_istrue(pl_block->condition,PL_COND_FLAG_FEED_REVOLUTION);
